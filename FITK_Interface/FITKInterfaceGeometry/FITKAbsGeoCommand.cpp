...
namespace Interface
{
    ...
    void FITKGeoCommandManager::reGenerate(int fromDataId)
    {
        // 获取更新数据的源命令索引。
        int index = 0;
        if (fromDataId != -1)
        ...
        for (int i = index; i < this->getDataCount(); i++)
        {
            ...
            FITKAbsGeoCommand* finalCmd = cmd->getFinalCommand();
            if (!finalCmd)
            {
                continue;
            }
            ...
            // 跳过多次编辑且已被刷新的命令。
            int finalId = finalCmd->getDataObjectID();
            if (updatedIds.contains(finalId))
            {
                continue;
            }

            finalCmd->update();
            updatedIds.push_back(finalId);
        }
    }
