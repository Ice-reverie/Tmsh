...
void FITKGraphActor::setRelativeCoincidentTopologyPolygonOffsetParameters(double val)
{
    if (this->Mapper)
    {
        // Use constant units offset (factor=0) to avoid slope-dependent depth bias banding (z-fighting stripes).
        this->Mapper->SetRelativeCoincidentTopologyPolygonOffsetParameters(0.0, val);
        this->Mapper->SetRelativeCoincidentTopologyLineOffsetParameters(0.0, val);
