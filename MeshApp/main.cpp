...
int main(int argc, char *argv[])
{
    ...
    app.setWindowIcon(QIcon(":/icons/APPMesh.ico"));
    ...
    app.setApplicationName("FastCAE-Mesh");
    app.setPluginKey("FastCAE_Mesh");
    // 注册程序的主要组件和设置
    ...
    app.regAppSettings(new MeshAPPSettings);             // 注册应用结构设置
    app.regCommandLineHandler(new CommandLineHandler);   // 注册命令行处理
    app.addGolbalSignalProcesser(new SignalProcessor);   // 信号处理器
    app.regWorkBenchHandler(new MeshAppWorkBenchHandler); //workbench处理
