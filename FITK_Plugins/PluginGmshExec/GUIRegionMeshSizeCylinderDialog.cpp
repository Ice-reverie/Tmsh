...
namespace GUI {
    GUIRegionMeshSizeCylinderDialog::GUIRegionMeshSizeCylinderDialog(Core::FITKActionOperator* oper, QWidget* parent)
        :Core::FITKDialog(parent), _oper(oper)
    {
        ...
        {
            connect(pickProvider, &Graph::GraphPickProvider::sig_pickedOver, this, &GUIRegionMeshSizeCylinderDialog::pickFinishedOper);
        }
        this->init();
    }
    ...
    { 
        setWindowTitle(tr("RegionMeshSizeCylinder"));

    }

    void GUIRegionMeshSizeCylinderDialog::pickFinishedOper()
    {
        //获取拾取数据
        Graph::GraphPickProvider* provider = Graph::GraphPickProvider::getInstance();
        if (provider == nullptr)return;
    ...
    void GUIRegionMeshSizeCylinderDialog::enablePickSig(Graph::PickObjType objType, Graph::PickMethod methodType)
    {
        Graph::GraphDataPickInfoStru pickInfo = Graph::GraphDataPickInfo::GetPickInfo();
