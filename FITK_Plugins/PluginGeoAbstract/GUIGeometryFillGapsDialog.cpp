#include "GUIGeometryFillGapsDialog.h"
#include "ui_GUIGeometryFillGapsDialog.h"
...
#include <QMessageBox>
namespace GUI {
    GUIGeometryFillGapsDialog::GUIGeometryFillGapsDialog(Core::FITKActionOperator* oper, QWidget* parent)
        :Core::FITKDialog(parent), _oper(oper)
    {
        _ui = new Ui::GUIGeometryFillGapsDialog();
        _ui->setupUi(this);
        // 设置对话框关闭时自动删除
        ...
        {
            connect(pickProvider, &Graph::GraphPickProvider::sig_pickedData, this, &GUIGeometryFillGapsDialog::pickFinishedOper);
        }
        this->init();
    }

    GUIGeometryFillGapsDialog::~GUIGeometryFillGapsDialog()
    {
        if (_ui) {
            delete _ui;
            _ui = nullptr;
    ...
    void GUIGeometryFillGapsDialog::init()
    {
        setWindowTitle(tr("Fill GAps"));
        //创建几何数据
        Interface::FITKInterfaceGeometryFactory* geoFactory = Interface::FITKInterfaceGeometryFactory::getInstance();
        if (!geoFactory) return;
        _obj = geoFactory->createCommand(Interface::FITKGeoEnum::FITKGeometryComType::FGTFillGapsFace);
        if (!_obj) return;
        //获取几何管理器
        _geometryData = FITKAPP->getGlobalData()->getGeometryData<ModelData::GeometryManager>();
         
        if (_geometryData == nullptr) return;
        QString name = _geometryData->checkName(tr("FillGAps-1"));
        _ui->lineEdit_Name->setText(name);
    }

    QString GUIGeometryFillGapsDialog::getNameText()
    {
        return _ui->lineEdit_Name->text();
    }

    void GUIGeometryFillGapsDialog::pickFinishedOper()
    {
    ...
    void GUIGeometryFillGapsDialog::createFillGapsData()
    {
        Interface::FITKAbsGeoModelOperFace* fillGapsModel = dynamic_cast<Interface::FITKAbsGeoModelOperFace*>(_obj);
        if (fillGapsModel == nullptr) return;
        //设置数据
        fillGapsModel->set(_lineVirtualTopos);
        fillGapsModel->setType(Interface::FITKAbsGeoModelOperFace::GeoFaceOperType::GBTFillGaps);
        for (Interface::VirtualShape addShape : _lineVirtualTopos)
        {
            fillGapsModel->addReferenceCmdID(addShape.CmdId);
        }
        _objID = fillGapsModel->getDataObjectID();
    }

    void GUIGeometryFillGapsDialog::on_pushButton_OK_clicked()
    {
    ...
    void GUIGeometryFillGapsDialog::on_pushButton_Cancel_clicked()
    {
        this->reject();
    }

    void GUIGeometryFillGapsDialog::on_line_pick_clicked()
    {
        //设置拾取方式
        enablePickSig(Graph::PickObjType::PickGeoCurve, Graph::PickMethod::PMIndividually);
    }

    void GUIGeometryFillGapsDialog::enablePickSig(Graph::PickObjType objType, Graph::PickMethod methodType)
    {
        Graph::GraphDataPickInfoStru pickInfo = Graph::GraphDataPickInfo::GetPickInfo();
