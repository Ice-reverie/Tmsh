#include "GUIGeometryMergeLineDialog.h"
#include "ui_GUIGeometryMergeLineDialog.h"
...
#include <QMessageBox>
namespace GUI {
    GUIGeometryMergeLineDialog::GUIGeometryMergeLineDialog(Core::FITKActionOperator* oper, QWidget* parent)
        :Core::FITKDialog(parent), _oper(oper)
    {
        _ui = new Ui::GUIGeometryMergeLineDialog();
        _ui->setupUi(this);
        // 设置对话框关闭时自动删除
        ...
        {
            connect(pickProvider, &Graph::GraphPickProvider::sig_pickedData, this, &GUIGeometryMergeLineDialog::pickFinishedOper);
        }
        this->init();
    }

    GUIGeometryMergeLineDialog::~GUIGeometryMergeLineDialog()
    {
        if (_ui) {
            delete _ui;
            _ui = nullptr;
    ...
    void GUIGeometryMergeLineDialog::init()
    {
        setWindowTitle(tr("Merge Line"));
        //创建几何数据
        Interface::FITKInterfaceGeometryFactory* geoFactory = Interface::FITKInterfaceGeometryFactory::getInstance();
        if (!geoFactory) return;
        _obj = geoFactory->createCommand(Interface::FITKGeoEnum::FITKGeometryComType::FGTMergeLine);
        if (!_obj) return;
        //获取几何管理器
        _geometryData = FITKAPP->getGlobalData()->getGeometryData<ModelData::GeometryManager>();
         
        if (_geometryData == nullptr) return;
        QString name = _geometryData->checkName(tr("MergeLine-1"));
        _ui->lineEdit_Name->setText(name);
    }

    QString GUIGeometryMergeLineDialog::getNameText()
    {
        return _ui->lineEdit_Name->text();
    }

    void GUIGeometryMergeLineDialog::pickFinishedOper()
    {
        _lineVirtualTopos.clear();
        //获取拾取数据
        Graph::GraphPickProvider* provider = Graph::GraphPickProvider::getInstance();
        if (provider == nullptr)return;
        ...
        for (Graph::GraphPickData* pickData : pickList) {
            ...
            for (int i : pickData->getPickedIDs())
            {
                if (OtherIDs.contains(i) == true)
                {
                    ...
                    _lineVirtualTopos.push_back(temp);
                }
            }
        }
        if (_lineVirtualTopos.isEmpty()) return;
        _ui->label_Num->setText(QString("Picked (%1)").arg(QString::number(_lineVirtualTopos.size())));
    }

    void GUIGeometryMergeLineDialog::createMergeLineData()
    {
        Interface::FITKAbsGeoModelOperLine* mergeLineModel = dynamic_cast<Interface::FITKAbsGeoModelOperLine*>(_obj);
        if (mergeLineModel == nullptr) return;
        //设置数据
        mergeLineModel->set(_lineVirtualTopos);
        mergeLineModel->setType(Interface::FITKAbsGeoModelOperLine::GeoLineOperType::GBTMerge);
        for (Interface::VirtualShape addShape : _lineVirtualTopos)
        {
            mergeLineModel->addReferenceCmdID(addShape.CmdId);
        }
        _objID = mergeLineModel->getDataObjectID();
    }

    void GUIGeometryMergeLineDialog::on_pushButton_OK_clicked()
    {
    ...
    void GUIGeometryMergeLineDialog::on_pushButton_Cancel_clicked()
    {
        this->reject();
    }

    void GUIGeometryMergeLineDialog::on_line_pick_clicked()
    {
        //设置拾取方式
        enablePickSig(Graph::PickObjType::PickGeoCurve, Graph::PickMethod::PMIndividually);
    }

    void GUIGeometryMergeLineDialog::enablePickSig(Graph::PickObjType objType, Graph::PickMethod methodType)
    {
        Graph::GraphDataPickInfoStru pickInfo = Graph::GraphDataPickInfo::GetPickInfo();
