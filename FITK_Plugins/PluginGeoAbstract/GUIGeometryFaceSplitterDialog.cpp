#include "GUIGeometryFaceSplitterDialog.h"
#include "ui_GUIGeometryFaceSplitterDialog.h"
...
#include <QMessageBox>
namespace GUI {
    GUIGeometryFaceSplitterDialog::GUIGeometryFaceSplitterDialog(Core::FITKActionOperator* oper, QWidget* parent)
        :Core::FITKDialog(parent), _oper(oper)
    {
        _ui = new Ui::GUIGeometryFaceSplitterDialog();
        _ui->setupUi(this);
        // 设置对话框关闭时自动删除
        ...
        {
            connect(pickProvider, &Graph::GraphPickProvider::sig_pickedOver, this, &GUIGeometryFaceSplitterDialog::pickFinishedOper);
        }
        this->init();
    }

    GUIGeometryFaceSplitterDialog::~GUIGeometryFaceSplitterDialog()
    {
        if (_ui) {
            delete _ui;
            _ui = nullptr;
    ...
    void GUIGeometryFaceSplitterDialog::init()
    {
        setWindowTitle(tr("Face Splitter"));

        _ui->comboBox_Tool->addItem(tr("Curve"), int(Graph::PickObjType::PickGeoCurve));
        _ui->comboBox_Tool->addItem(tr("Surface"), int(Graph::PickObjType::PickGeoFace));

        Interface::FITKInterfaceGeometryFactory* geoFactory = Interface::FITKInterfaceGeometryFactory::getInstance();
        if (!geoFactory) return;
        _obj = geoFactory->createCommand(Interface::FITKGeoEnum::FITKGeometryComType::FGTSurfaceSplitter);
        if (!_obj) return;
        //获取几何管理器
        _geometryData = FITKAPP->getGlobalData()->getGeometryData<ModelData::GeometryManager>();
        if (_geometryData == nullptr) return;
        QString name = _geometryData->checkName(tr("FaceSplitter-1"));
        _ui->lineEdit_Name->setText(name);
    }

    QString GUIGeometryFaceSplitterDialog::getNameText()
    {
        return _ui->lineEdit_Name->text();
    }

    void GUIGeometryFaceSplitterDialog::pickFinishedOper()
    {
        //获取拾取数据
        Graph::GraphPickProvider* provider = Graph::GraphPickProvider::getInstance();
        if (provider == nullptr)return;
        ...
        else if (_pickFlag == PickFlag::Tool)
        {
            ...
            if (Graph::PickObjType(_ui->comboBox_Tool->currentData().toInt()) == Graph::PickObjType::PickGeoCurve)
                _virtualTool.Type = Interface::FITKGeoEnum::VTopoShapeType::VSEdge;
            else if (Graph::PickObjType(_ui->comboBox_Tool->currentData().toInt()) == Graph::PickObjType::PickGeoFace)
    ...
    void GUIGeometryFaceSplitterDialog::createFaceSplitterData()
    {
        Interface::FITKAbsGeoSurfaceSplitter *faceSplitterOper = dynamic_cast<Interface::FITKAbsGeoSurfaceSplitter *>(_obj);
        if (faceSplitterOper == nullptr) return;
        faceSplitterOper->setSourceShape(_virtualTarget);
        faceSplitterOper->setToolShape(_virtualTool);
        faceSplitterOper->addReferenceCmdID(_virtualTarget.CmdId);
        _objID = faceSplitterOper->getDataObjectID();
    }

    void GUIGeometryFaceSplitterDialog::on_pushButton_OK_clicked()
    {
    ...
    void GUIGeometryFaceSplitterDialog::on_pushButton_Cancel_clicked()
    {
        this->reject();
    }

    void GUIGeometryFaceSplitterDialog::on_Target_pick_clicked()
    {
        //设置拾取方式
    ...
    void GUIGeometryFaceSplitterDialog::on_Tool_pick_clicked()
    {
        //设置拾取方式
        _pickFlag = PickFlag::Tool;
        enablePickSig(Graph::PickObjType(_ui->comboBox_Tool->currentData().toInt()), Graph::PickMethod::PMSingle);
    }

    void GUIGeometryFaceSplitterDialog::enablePickSig(Graph::PickObjType objType, Graph::PickMethod methodType)
    {
        Graph::GraphDataPickInfoStru pickInfo = Graph::GraphDataPickInfo::GetPickInfo();
