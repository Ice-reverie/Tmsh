...
namespace Comp
{
    ...
    {        
        ...
        _infoColor.setNamedColor("#808080");
    ...
    void FITKConsole::displayMessage(MessageType type, const QString message)
    {
        ...
        {
            const QString safe = QString(message).toHtmlEscaped().replace("\n", "<br/>");
            value = QString("<span style=\" font-family: %1; font-size:%2pt; color:rgb(%3, %4, %5); \">%6</span>")
                .arg(ft.family()).arg(ft.pointSize()).arg(_infoColor.red()).arg(_infoColor.green()).arg(_infoColor.blue())
                .arg(safe);
            break;
        }
        default: break;
    ...
    void FITKConsole::setInfoTextColor(QColor color)
    {
        _infoColor = color;
    }
