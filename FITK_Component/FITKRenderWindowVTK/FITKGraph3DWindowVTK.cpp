...
namespace Comp
{
        ...
        :_initializer(ini)
    {
        ...
        for (int i = 0; i < renderNum; i++)
        {
            ...
            if (rendervtk)
            {
                rendervtk->SetLayer(i);

                // VTK 的多 renderer 分层渲染默认每个 renderer 会清空深度缓冲，
                // 这会导致上层(如网格/拾取)不受下层(几何)遮挡，出现“背面网格透出”。
                // 对非 0 层 renderer 开启 PreserveDepthBuffer，使其复用下层深度缓冲进行深度测试。
                if (i > 0)
                {
                    rendervtk->PreserveColorBufferOn();
                    rendervtk->PreserveDepthBufferOn();
                }
                else
                {
                    rendervtk->PreserveColorBufferOff();
                    rendervtk->PreserveDepthBufferOff();
                }
            }
