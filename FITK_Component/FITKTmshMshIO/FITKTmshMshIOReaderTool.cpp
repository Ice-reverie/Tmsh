...
namespace Tmsh
{
    ...
    //类的接口实现
    FITKTmshMshIOReaderTool::FITKTmshMshIOReaderTool(Interface::FITKUnstructuredMesh * mesh, Interface::FITKComponentManager * setSurfManager)
    {
        _mesh = mesh;
        _setSurfManager = setSurfManager;
    }
    void FITKTmshMshIOReaderTool::addNode(int id, double x, double y, double z)
    {
        _nodes.append(Tmsh::TmshNode(id, x, y, z));
    }
    void FITKTmshMshIOReaderTool::addElement(int id, int eleType, QList<int> nodeIDs)
    {
        int index = _elements.size();
        _elements.append(Tmsh::TmshElement(id, eleType, nodeIDs));
        _elemMapping.insert(id, index);
    }
    void FITKTmshMshIOReaderTool::addElementSet(int dim, QList<int> elementIDs)
    {
        _elementSets.append(Tmsh::TmshSet(dim, elementIDs));
        if (dim > _maxDim)
            _maxDim = dim;
    }
    void FITKTmshMshIOReaderTool::update(bool isFilterElement)
    {
        if (!_mesh) return;
        if (isFilterElement)
        {
    ...
    void FITKTmshMshIOReaderTool::createSetData(QList<int> elemIdList)
    {
        if (elemIdList.isEmpty() || !_setSurfManager) return;
        QString name = _setSurfManager->checkName("TmshSet-1");
    ...
    void FITKTmshMshIOReaderTool::startFilterElement()
    {
    ...
    void FITKTmshMshIOReaderTool::vtkInitMaxDimElement(vtkUnstructuredGrid* mesh, QHash<int, int>* nodeMapping, QHash<int, int>* elementMapping)
    {
        //获取所有的最高单元
    ...
    void FITKTmshMshIOReaderTool::vtkFilterLowDimElement(int dim, vtkUnstructuredGrid * mesh, QHash<int, int>* nodeMapping, QHash<int, int>* elementMapping)
    {
