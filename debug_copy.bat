...
rem Copies 3rd-party runtime files into the Debug output folder.
rem Target: output\bin-debug
rem Rules:
rem - Traverse Tools\Win64\<vendor>\...
rem - For each vendor, copy its "bind" folder (vendor\bind or vendor\<ver>\bind) into output\bin-debug
rem - If no bind folder exists, skip
rem - Python37: copy vendor\Lib and vendor\*.dll into output\bin-debug
rem - gmsh: no bind folder, always copy vendor\bin into output\bin-debug
...
for /d %%D in ("%TOOLS%\*") do (
  ...
  if /I "!VENDOR_NAME!"=="OpenGL" (
    ...
    call :copy_vendor_level "!VENDOR_DIR!" "!VENDOR_NAME!" "%DEST%" "bind"
  )
)
